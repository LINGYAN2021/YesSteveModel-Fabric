...
//        EntityCapabilityHolder.get(player, ModelInfoCapabilityProvider.MODEL_INFO_CAP)
//                .ifPresent(capability -> update.setRevision(capability.nextStateRevision()));
    ...
    public static void handleReport(PlayerStateV0.PlayerStateReport report,
        ...
        var stateSession = GameServerPlayerStateSession.active(context.connection()).orElse(null);
        ...
        context.enqueueWork(() -> EntityCapabilityHolder.get(sender, ModelInfoCapabilityProvider.MODEL_INFO_CAP)
    ...
    public static boolean sendAuthoritativeFull(ServerPlayer player, boolean broadcast) {
        var capability = EntityCapabilityHolder.get(player, ModelInfoCapabilityProvider.MODEL_INFO_CAP)
                .orElse(null);
        if (capability == null) {
            return false;
        }
