        ...
        if (entry != null) {
            ...
            if (modelHash != null) {
                YesSteveModel.LOGGER.warn(
                        "In-use model is absent from the client catalog hash={} catalogModels={} serverRevision={}",
                        modelHash, service.catalog().models().size(), service.catalog().serverRevision());
            }
                    ...
                    Minecraft.getInstance().execute(() -> {
                        ...
                        if (lease == null) {
                            if (generation == requestGeneration && desired.equals(requestedRenderTarget)) {
                                YesSteveModel.LOGGER.warn(
                                        "Failed to acquire in-use model render target hash={} target={} texture={}",
                                        desired.modelHash(), desired.renderTargetId(), desired.textureName(),
                                        error);
