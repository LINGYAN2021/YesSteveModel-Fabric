...
public class FishingHookRendererReplace {
    public static boolean renderInMixin(FishingHook entity, float yaw, float partialTick, PoseStack poseStack,
        ...
        return EntityCapabilityHolder.get(entity, ProjectileAnimatableCapabilityProvider.CAP).map(cap -> {
            if (cap.isInitialized() && cap.isModelPresent()) {
                ...
                if (player != null) {
                    // 26.1.2：lineStrip 渲染类型已移除，改用线条列表（每段两个顶点）
                    collector.submitCustomGeometry(poseStack, RenderTypes.lines(),
                            (pose, consumer) -> renderFishingLine(entity, partialTick, consumer, pose, player));
                }
                return false;
            }
            return true;
    ...
    private static void renderFishingLine(FishingHook hook, float partialTicks, VertexConsumer buffer, PoseStack.Pose last, Player player) {
        // 确定钓竿持握方向
        ...
        if ((options != null && options.getCameraType().isFirstPerson()) && player == Minecraft.getInstance().player) {
            ...
            Vec3 cameraPlanePoint = dispatcher.camera.getNearPlane(partialTicks).getPointOnPlane((float) handDirection * 0.525F, -0.1F);
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            stringVertex(x, y, z, buffer, last,
                    fraction(i + 1), fraction(Math.min(i + 2, 16)),
                    color[0], color[1], color[2]);
        }
