...
public final class BakedAnimationCache {
    ...
    public AnimationStore loadOrBake(Hash256 modelHash, Hash256 descriptorHash, String targetId,
        ...
        // setHash 必须包含 targetId：同一模型的多个渲染目标（如 player 与多个 vehicle）
        // 可能共享同一描述符 blob（definitionHash 相同）且动画集同名，否则它们的缓存
        // 会落在同一路径上互相隔离/重写（乒乓），读取方会在写入窗口期遇到文件缺失
        var setHash = animationSetHash(definitionHash, targetId, animationSet);
    ...
    static ArrayList<AnimationOuterClass.Animation> flatten(
        ...
        // 旧版 2.6.5 的语义是"同名动画后写覆盖前写"：模型作者经常把内置 arm 文件
        // 原样拷进自己的包（use_mainhand:eat 等），跨文件重名是合法的。
        // 之前按全局唯一硬性报错会让整只模型加载失败（表现为卡片空白）。
        // 这里改为跳过空名、重名时保留后出现的定义，仅对空名记日志。
        var seen = new HashSet<String>();
        int blank = 0;
        for (var file : animationFiles) {
            for (var animation : file.getValue().getAnimations()) {
                if (animation.getName().isBlank()) {
                    blank++;
                    continue;
                }
                if (!seen.add(animation.getName())) {
                    var previous = result.stream()
                            .filter(existing -> existing.getName().equals(animation.getName()))
                            .findFirst();
                    previous.ifPresent(result::remove);
                }
                result.add(animation);
            }
        }
        if (blank > 0) {
            YesSteveModel.LOGGER.warn("Skipped {} blank animation name(s) in animation set", blank);
        }
        result.sort(Comparator.comparing(AnimationOuterClass.Animation::getName));
        return result;
    }

    static Hash256 animationSetHash(Hash256 definitionHash, String targetId, String animationSet) {
        var targetBytes = targetId.getBytes(StandardCharsets.UTF_8);
        var setName = animationSet.getBytes(StandardCharsets.UTF_8);
        var input = ByteBuffer.allocate(HASH_DOMAIN.length + Hash256.SIZE
                        + Integer.BYTES + targetBytes.length + Integer.BYTES + setName.length)
                .put(HASH_DOMAIN).put(definitionHash.bytes())
                .putInt(targetBytes.length).put(targetBytes)
