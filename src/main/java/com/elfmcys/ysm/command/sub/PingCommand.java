...
public class PingCommand {
    ...
    private static int ping(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        player.sendSystemMessage(Component.translatable("message.yes_steve_model.client.ping_result", FabricLoader.getInstance().getModContainer(YesSteveModel.MOD_ID).map(c -> c.getMetadata().getVersion().getFriendlyString()).orElse("unknown")));
