        ...
        for (var animationType : PLAYER_MAIN_ANIMATION_TYPES) {
            ...
            if (path != null) {
                ...
                if (!dryRun) {
                    ...
                    // 老版模型包通常不写 extra_animation（信息在 info.json 里，甚至根本没有），
                    // 于是轮盘没有入口、按 Z 打不开。这里用 extra 动画文件里的动画名兜底：
                    // 老格式的动画名本身就是 extra0..extraN，正好可以直接当作轮盘键值。
                    if ("extra".equals(animationType) && properties.extraAnimation.isEmpty()) {
                        for (var extra : animation.getAnimations()) {
                            if (properties.extraAnimation.size() >= 8) {
                                break;
                            }
                            var extraName = extra.getName();
                            if (StringUtils.isNotBlank(extraName)) {
                                properties.extraAnimation.put(extraName, extraName);
                            }
                        }
                    }
