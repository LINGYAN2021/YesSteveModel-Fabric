...
public final class ModelInfoCapability {
    ...
    public void deserializeNBT(CompoundTag tag) {
        // serializeNBT 写的是 model_hash，早期读的键名不一致会让玩家重登后丢失模型选择，
        // 只留下上一次的 select_texture，从而向客户端同步出错误的（模型，贴图）组合
        var storedHash = tag.getString("model_hash").orElseGet(
                () -> tag.getString("model_id").orElse(""));
        modelId = storedHash.isEmpty() ? null : Hash256.parse(storedHash);
        selectTexture = tag.getString("select_texture").orElse("");
        ...
        mandatory = tag.getBoolean("mandatory").orElse(false);
        disabled = tag.getBoolean("disabled").orElse(false);
        roamingVariables.deserialize(tag.getCompound("molang_storage").orElseGet(CompoundTag::new));
    }
}
