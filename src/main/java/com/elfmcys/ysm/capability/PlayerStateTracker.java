...
public class PlayerStateTracker extends HumanoidStateTracker<Player> {
    ...
    private final Object2ByteOpenHashMap<Holder<MobEffect>> effects;
    ...
    public void updateProtocolState(PlayerStateV0.GameplayState gameplay,
        ...
        if (effectState != null) {
            ...
            for (var value : effectState.getEffects()) {
                ...
                var effect = BuiltInRegistries.MOB_EFFECT.get(key).orElse(null);
                if (effect != null) {
    ...
    public byte getEffectLevel(Holder<MobEffect> effect) {
        if (localPlayer) {
